...
public class AccountRoleController extends BaseController {
	...
	public void save(){
		int type=Db.queryBigDecimal("select count(1) from st_role where cid=? ",getCid()).intValue()+2;
		Db.save(tableName,getRecord().remove("oper").set("id", "roleSequ.nextval").set("type", type));
	}
	/**
	 *type类型大于5才能删除角色
	 */

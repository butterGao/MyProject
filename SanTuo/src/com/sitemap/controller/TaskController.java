...
public class TaskController extends BaseController {
	...
	public void show() {
		Page<Record> rs;
		if(getCid()==0){
			rs=Db.paginate(getParaToInt("page"), getParaToInt("rows"), "select *", "from " + tableName+" order by id desc");

		}else{
			rs=Db.paginate(getParaToInt("page"), getParaToInt("rows"), "select *", "from " + tableName+" where cid=? order by id desc",getCid());

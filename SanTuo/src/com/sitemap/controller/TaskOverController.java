	...
	public void show(){
		if(getCid()==0){
			Page<Record>p=Db.paginate(getParaToInt("page"), getParaToInt("rows"),
					...
					"from ST_TASK_CONTENT a,ST_TASK_OVER b,st_task c where a.ID=b.CID and c.id=a.tid order by a.id desc");
			renderJson(p);
		}else{
			Page<Record>p=Db.paginate(getParaToInt("page"), getParaToInt("rows"),
					"select b.*,a.TITLE, a.TASKDATA ,c.name as tname,c.address,c.STARTTIME,c.ENDTIME ",
					"from ST_TASK_CONTENT a,ST_TASK_OVER b,st_task c where a.ID=b.CID and c.id=a.tid and a.TID in(select id from ST_TASK where cID = ?) order by a.id desc",getCid());
			renderJson(p);
		}
	}
}
